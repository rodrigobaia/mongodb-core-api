...
{
    public class AuditoriaBusiness : BusinessBase, IAuditoriaBusiness
    {
        private readonly IMapper _mapper;
        private readonly IAuditoriaRepository _auditoriaRepository;

        public AuditoriaBusiness(IMapper mapper,
        ...
        public async Task<long> DeleteAsync(AuditoriaGetDTO auditoriaGetDTO)
        {
            _auditoriaRepository.DatabaseName = DatabaseName;
        ...
        public async Task<AuditoriaGetDTO> GetByIdAsync(string id)
        {
            _auditoriaRepository.DatabaseName = DatabaseName;
            ...
            var auditoriaGetDTO = _mapper.Map<AuditoriaGetDTO>(audity);
            return auditoriaGetDTO;
        }

        public async Task<long> InsertAsync(AuditoriaInsertDTO auditoriaInsertDTO)
        {
            var auditoria = _mapper.Map<Auditoria>(auditoriaInsertDTO);
            _auditoriaRepository.DatabaseName = DatabaseName;
            var results = await _auditoriaRepository.InsertAsync(auditoria);

            auditoriaInsertDTO = _mapper.Map<Auditoria, AuditoriaInsertDTO>(auditoria);
        ...
        public async Task<IEnumerable<AuditoriaGetDTO>> ListByPeriodAsync(DateTime initDate,
                                                                            DateTime endDate,
                                                                            TimeSpan iniTime,
                                                                            TimeSpan endTime)
        {
            var lstResults = new List<AuditoriaGetDTO>();
            ...
            lstResults = _mapper.Map<List<AuditoriaGetDTO>>(lst1);
            await _auditoriaRepository.DropCollectionAsync(filterTable);
        ...
        public async Task<AuditoriaGetDTO> UpdateAsync(AuditoriaUpdateDTO auditoriaUpdateDTO)
        {
            _auditoriaRepository.DatabaseName = DatabaseName;

            var audity = _mapper.Map<Auditoria>(auditoriaUpdateDTO);
